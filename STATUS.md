...
Required scopes now materialize eligible immutable records and exported subrecords.
Aliases, projections and scalar field reads retain shapes and leaf kinds without runtime
locals. Binding a source checks complete ancestor evidence; subsequent alias reads reuse
the materialized value. Record copies share the existing type-node budget.

Commits: `83cf6e3` (source lookup), `978b40e` (materialization), `02fdfa5` (integration).
The [supported guide](compiler/docs/COMPUTED_TYPES.md#required-record-scratch)
covers source versus alias work, annotations, scopes and namespace boundaries.
...
- `python3 -B tools/verify.py --compiler`: all ten checks passed, including 1579
  Rust tests (769 library/810 native), 20 Python tests, fmt, Clippy and build.
- Four checker tests and five native groups pass, covering record shapes, leaf values,
  ancestor errors, budgets, alias work, documentation and silent module initialization.
...
| --- | --- |
| Compiler | Required record scratch materializes checked source evidence without runtime locals. |
...
1. Plan explicitly annotated required record construction with bounded immutable shapes,
   typed field emissions, initialization checks and shared budgets. See the
   [compiler handoff](compiler/STATUS.md#next-steps).
2. Keep whole-module records, unannotated construction, skipped documented declarations,
